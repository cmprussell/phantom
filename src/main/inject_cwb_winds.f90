...
!
 ...
 integer, parameter :: n_wind_prop = 12
...
subroutine init_inject(ierr)
 ...
 allocate(iwindorig_to_ict(0:nptmass)) !index zero is for unknown/init particles
 ...
 !wind acceleration data
 ! units of the following quantities:
 ! wind(i_lum,i)     : solar luminosity
 ! xyzmh_ptmass(4,i) : code mass
 ! wind(i_vel,i)     : km/s
 ! wind(i_rad,i)     : solar radius
 !
 ! kappac            : cgs
 ! kappar            : cgs
 !
 !Note: Eqs. A9 and A5 both have the cancellatory factors of 4*pi*c removed to reduce numerical roundoff errors
 do i=1,nptmass
    j = i-nskip_ptmass
    !constant factor of Eq. A9 (Madura+13), which is then multiplied by kappa_bar in externf_windaccel()
    !wind(i_xantgrav,j) = wind(i_lum,j)*solarl/(4.d0*pi*gg*xyzmh_ptmass(4,i)*umass*c)
    wind(i_xantgrav,j) = wind(i_lum,j)*solarl/(gg*xyzmh_ptmass(4,i)*umass) * udist**3/unit_energ/utime
    !                    erg/s              * (g*s^2/cm^3) / g             = erg*s/cm^3
    if (wind(i_lum,j)>0.d0) then
       !constant component of Eq. A5 (Madura+13)
       !wind(i_kappac,j) = 4.d0*pi*c/(wind(i_lum,j)*solarl) * gg*xyzmh_ptmass(4,i)*umass
       wind(i_kappac,j) = 1.d0/(wind(i_lum,j)*solarl) * gg*xyzmh_ptmass(4,i)*umass * unit_energ*utime/udist**3
       !                  s/erg                       * (cm^3/g/s^2) *g            = cm^3/erg/s
       !Note: if G=1 by unit definition, then the following also works for kappac (since gg * utime**2*umass/udist**3 === 1.0)
       !wind(i_kappac,j) = 1.d0/(wind(i_lum,j)*solarl) * xyzmh_ptmass(4,i)*umass * unit_energ/utime/umass
       !!                  s/erg                       * g                  = g*s/erg
       !radius-dependent component of Eq. A5 (Madura+13), with specific radius term to come in extern_windaccel()
       !wind(i_kappar,j) = 4.d0*pi*c/(wind(i_lum,j)*solarl) * (wind(i_vel,j)*1.d5)**2*wind(i_rad,j)*solarr*wind(i_vbeta,j)
       wind(i_kappar,j) = 1.d0/(wind(i_lum,j)*solarl) * (wind(i_vel,j)*1.d5)**2*wind(i_rad,j)*solarr*wind(i_vbeta,j) * unit_energ*utime/udist**3
       !                  s/erg                       * cm^2/s^2               *cm                                   = cm^3/erg/s
    else
       wind(i_kappac,j) = 0.
       wind(i_kappar,j) = 0.
    endif
    !windaccel is located in the "part" module, which is what wil be referenced in extern_windaccel()
    windaccel(ixantgrav,i) = wind(i_xantgrav,j)
    windaccel(ikappac  ,i) = wind(i_kappac  ,j)
    windaccel(ikappar  ,i) = wind(i_kappar  ,j)
    windaccel(ivbeta   ,i) = wind(i_vbeta   ,j)

    xyzmh_ptmass(ihacc,i) = wind(i_rad   ,j)*solarr/udist  !accretion
    xyzmh_ptmass(iReff,i) = wind(i_radinj,j)*solarr/udist  !wind injection
    xyzmh_ptmass(ilum ,i) = wind(i_lum   ,j)
 enddo
 ...
 !time interval = max mass to inject per timestep / max mass-loss rate [in code
 !units]
 if (ninjectmax_cwb>0) then
    dtinject_cwb = ninjectmax_cwb*massoftype(igas) / (maxval(wind(i_Mdot,1:nptmass)) * (solarm/umass)*(utime/years))
 else
    dtinject_cwb = min(1.e99,0.1*huge(dtinject_cwb)) !prevents 3-digit exponents, which fortran writes weirdly
 endif
...
subroutine inject_particles(time,dtlast,xyzh,vxyzu,xyzmh_ptmass,vxyz_ptmass,&
 ...
 use part,      only:massoftype,igas,ihacc,iReff,i_tlast,iwindorig,isdead_or_accreted,iphase,iunknown,eos_vars,imu,itemp
...
!print*, j,i,nskip_ptmass,wind(i_Mdot,j),wind(i_vel,j),' -- ',deltat,time,tlast,Minject
...
!+
!-----------------------------------------------------------------------
subroutine read_wind_data(datafile_wind,nstars)
 use io,      only:error,fatal
 use options, only:use_var_comp
 use part,    only:n_startypes,mu_startypes,habund_startypes,name_startypes
 use datafiles, only:find_phantom_datafile
 character(len=*), intent(in)  :: datafile_wind
 integer,          intent(out) :: nstars
 integer :: iunit,ierr,i,j
 character(len=len(datafile_wind)) :: filename_wind

 print "(a,i0,a)", ' start read_wind_data(datafile_wind,nstars) = read_wind_data('//trim(datafile_wind)//',nstars)'
 !nstars is not yet defined, so don't write out its garbage value in the above line

 nstars = 0
 wind = 0.
 filename_wind=find_phantom_datafile(datafile_wind,'cwb')
 open(newunit=iunit,file=trim(filename_wind),status='old',action='read',iostat=ierr)
 if (ierr /= 0) then
    print*
    if (trim(datafile_wind) /= trim(filename_wind)) then
       print "(a)",' ERROR opening "'//trim(datafile_wind)//'" for read of wind data, and'
    endif
    print "(2(a,/),a)",' ERROR opening "'//trim(filename_wind)//'" for read of wind data', &
                     ' --> this file should contain id,vinf,Mdot,R,etc for each star, one per line', &
                     ' Stopping...'
    close(iunit)
    call fatal('read_wind_data',trim(filename_wind)//' not read in properly')
 endif
 ...
 if (use_var_comp) then
    ...
    do while(ierr == 0)
       ...
       if (nstars <= maxptmass) then
          ...
          !print "(2(a,i0))", ' nstars, ierr = ',nstars,', ',ierr
          if (ierr /= 0) then
    ...
    if (nstars > 0) then
       ...
       do i=1,nstars
          ...
          do j=16,len(trim(wind_char(i_fulltype_char,i))),-1
             write(*,'(a)',advance='no') ' '
          enddo
          write(*,'(1x,"|")')
       enddo
    ...
    do while(ierr == 0)
       ...
       if (nstars <= maxptmass) then
          ...
          wind_int(i_comp,1:nstars) = 1 !needs to be initialized for mu_startypes agrument in init_inject()
       else
          call error('read_wind_data','array bounds exceeded')
       endif
    enddo
...
!+
!----------------------------------------------------------------
subroutine read_use_var_comp_data(datafile_mhn)
 use io,   only:error,warning
 use part, only:n_startypes,mu_startypes,habund_startypes,name_startypes
 use eos,  only:gmw,num_var_comp
 use datafiles, only:find_phantom_datafile
 character(len=*), intent(in) :: datafile_mhn
 integer :: iunit,ierr
 integer :: i,j
 character(len=12) :: unknown_init_char
 character(len=20) :: unknown_init_name_startypes
 logical :: read_unknown_startypes,found_unknown_startypes
 real :: tol_mu = 1.d-6
 character(len=len(datafile_mhn)) :: filename_mhn

 print "(/,a)", ' start read_use_var_comp_data(datafile_mhn) = read_use_var_comp('//trim(datafile_mhn)//')'

 n_startypes = 0
 read_unknown_startypes=.false.
 found_unknown_startypes=.false.
 ...
 if (ierr /= 0) then
    print "(2(/,a))",' ERROR opening "'//trim(filename_mhn)//'" for read of various composition data', &
                       ' -> this file should contain mu,habund,name for each type of point mass, one per line'
 ...
 if (n_startypes==1) then
    print "(a,i0,a)",' read ',n_startypes,' various composition for point masses from '//trim(filename_mhn)
 else
 ...
 if (n_startypes>0) then
    ...
    print "(a,i0)", ' num_var_comp has been set to n_startypes, so num_var_comp = ',num_var_comp
 else
    print "(a)", ' no startypes were read in from '//trim(filename_mhn)
    print "(a,g0)", ' --> mu for all particles will come from module eos-->gmw, which is ',gmw
    print "(a,g0)", ' --> habund for all particles will be 0.7'
