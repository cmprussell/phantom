...
subroutine startrun(infile,logfile,evfile,dumpfile,noread)
 use dim,              only:maxp,maxalpha,nalpha,mpi,ind_timesteps,inject_parts,gr,cwb,dtinject_cwb
...
subroutine get_derivs_initial(time,dumpfile,ntot,dtnew_first,ierr)
 use dim,              only:maxalpha,maxp,nalpha,do_radiation,cwb
 use part,             only:npart,fxyzu,eos_vars,alphaind
 use deriv,            only:get_derivs_global
 use timestep,         only:dtmax,dtinject
#ifdef LIVE_ANALYSIS
...
!#endif
 if (cwb) nbinmax=max(nbinmax,ceiling(log(dtmax/dtinject)/log(2.))) !from dtinject=dtmax/2^nbinmax
 !
