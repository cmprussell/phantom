...
# SBATCH --mem=8G
# SBATCH --mem-per-cpu=1024M
#SBATCH --job-name=pha-TestSuite
# SBATCH --partition=standard
# SBATCH --partition=_workgroup_
#SBATCH --time=0-120:00:00
#SBATCH --mail-user='crussell@udel.edu'
#SBATCH --mail-type=END,FAIL,TIME_LIMIT_90
##SBATCH --export=NONE
#SBATCH --export=ALL
#UD_QUIET_JOB_SETUP=YES
...
#vpkg_require intel
#vpkg_require intel/2020u4 gcc/12.2
echo
...
sacct -j $SLURM_JOBID --format=JobID%50,JobName%50
echo

sacct -j $SLURM_JOBID --format=JobID,JobName,Submit,Start,TimeLimit,NCPUS,ReqMem,NodeList,Partition,QOS
echo

echo "Start time from 'date'"
srun date
echo
...
sacct -j $SLURM_JOBID --format=JobID%50,JobName%50,Partition,QOS
echo
sacct -j $SLURM_JOBID --format=JobID,AveDiskRead,MaxDiskRead,AveDiskWrite,MaxDiskWrite,AvePages,MaxPages
echo
sacct -j $SLURM_JOBID --format=JobID,ReqMem,AveRSS,MaxRss
echo
sacct -j $SLURM_JOBID --format=JobID,NCPUs,NNodes,UserCPU,SystemCPU,TotalCPU,CPUTime
echo
sacct -j $SLURM_JOBID --format=JobID,JobName,Submit,Start,End,Elapsed,TimeLimit 
